...
import 'package:ride_hailing/screens/home_screen.dart'; 
...
DriverInfo? _currentDriverInfo;
  ...
  Future<void> _fetchRideDetails() async {
    ...
    if (ride != null) {
      if (mounted) {
        setState(() {
          _pickupLocation = LatLng(ride.pickupLocationLat, ride.pickupLocationLng);
  ...
  void _startFirebaseListener() {
    ...
    const String databaseUrl = 'https://appride-f2bb5-default-rtdb.asia-southeast1.firebasedatabase.app';
    ...
    _firebaseSubscription = _rideRef!.onValue.listen((event) {
      ...
      if (rawData != null && rawData is Map) {
         ...
         if (data['driver_location'] != null) {
            ...
            double rotation = loc['bearing'] != null ? double.parse(loc['bearing'].toString()) : 0.0;
  ...
  void _updateStatusText() {
    switch (_currentStatus) {
      case 'accepted': _statusText = "Tài xế đang đến..."; break;
      case 'arrived': _statusText = "Tài xế đã đến điểm đón!"; break;
      case 'in_progress': _statusText = "Đang di chuyển..."; break;
      case 'completed': _statusText = "Đã đến nơi!"; break;
      case 'cancelled': _statusText = "Chuyến xe đã hủy"; break;
    }
  }
...
void _loadCustomMarker() async {
    try {
      final icon = await BitmapDescriptor.fromAssetImage(
        const ImageConfiguration(size: Size(48, 48)), // Kích thước mong muốn
        'assets/images/car_icon.png' // Đường dẫn ảnh của mày
      );
      setState(() {
        _driverIcon = icon;
      });
      ...
      builder: (ctx) => AlertDialog(
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () {
              Navigator.pop(ctx);
              _handleCancelRide(reasonController.text.isEmpty ? "Khách hủy" : reasonController.text);
            },
  ...
  Future<void> _handleCancelRide(String reason) async {
    ...
    if (success) {
      ...
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Đã hủy chuyến!")));
      Future.delayed(const Duration(seconds: 1), () {
         if (mounted) {
            // Chỉ cần pop là nó tự lùi về màn hình VehicleSelectionMapScreen đang nằm bên dưới
            Navigator.pop(context); 
         }
      });
    } else {
      ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("Lỗi hủy chuyến hoặc chuyến đã hoàn thành!")));
    }
  }
          ...
          TextButton(
            onPressed: () {
              ...
              Navigator.of(context).pushAndRemoveUntil(
                MaterialPageRoute(
                  builder: (context) => const HomeScreen(), // ⚠️ Đảm bảo tên Class màn hình chính là HomeScreen
                ),
  ...
  Color _getStatusColor(String status) {
    switch (status) {
      ...
      case 'in_progress': return AppTheme.primaryGreen;
              ...
              child: Column(
                ...
                children: [
                 ...
                 FloatingActionButton.small(
                  ...
                  const Divider(),
                  if (_currentStatus != 'completed' && _currentStatus != 'cancelled' && _currentStatus != 'in_progress')
                    Center(
                      child: _isCancelling
